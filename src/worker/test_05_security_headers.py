#!/usr/bin/env python3
"""
test_05_security_headers.py – Bravo6 Enterprise HTTP Security Policy Analyzer (v8.0)
====================================================================================
...
USER_AGENT = "Bravo6-SecurityHeaders/8.0"
...
# ── Finding & Scoring ───────────────────────────────────────────────────
def _make_finding(title, description="", severity="info", confidence=50, status="fail",
                  header="", header_value="", explanation="", evidence="",
                  remediation="", cwe="", owasp="", poc=None,
                  detection_method="Header Check", **kwargs):
    """
    Maps legacy arguments to the strict 10-field Bravo6 Plugin Contract v2.2 schema.
    Ensures exactly 10 keys are returned, 'status' is excluded, and 'header' is mapped to 'location'.
    """
    loc = header if header else "HTTP Response Headers"
    if header and header_value:
        loc = f"{header}: {header_value}"
        
    ev = evidence or explanation or description
...
def _check_cache_control(final_headers, set_cookie, url=""):
    ...
    if max_age_match and int(max_age_match.group(1)) >= 86400:
        ...
        if sev != "high": 
            sev = "medium" if set_cookie else "low"
...
def _check_hsts(headers_list=None, is_cdn=False, target_url="", final_headers=None):
    ...
    issues = []
    if max_age < 31536000: 
        issues.append(f"max-age={max_age} (< 1 year)")
    elif max_age < 63072000: 
        issues.append(f"max-age={max_age} (< 2 years, not preload ready)")
    if not incl_sub: 
...
# ══════════════════════════════════════════════════════════════════════════
# Main Scanner – STRICTLY uses shared_page & DI
# ══════════════════════════════════════════════════════════════════════════
async def run(url: str, **kwargs) -> Dict[str, Any]:
    try:
        ...
        # Determine if we need to fetch the page
        needs_fetch = True
        ...
        if needs_fetch:
            temp_session = False
            if not session:
                session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15), headers={"User-Agent": USER_AGENT})
                temp_session = True
            try:
                async with session.get(target) as resp:
                    ...
                    main_html = await resp.text() if resp.status == 200 else ""
            except Exception:
                final_headers = {}
                final_status = 0
                final_url = target
        ...
        if is_error_page:
            raw_findings.append(_make_finding(
                f"Response returned {final_status}",
        ...
        # Merge all findings (passing checks already have severity="info")
        ...
        if is_sensitive:
            ...
            for f in all_findings:
                loc = f.get("location", "")
                loc_header = loc.split(":")[0].strip() if loc else ""
                if loc_header in sensitive_headers and f["severity"] != "critical":
                    f["severity"] = boost_map.get(f["severity"], f["severity"])
                    
        # Filter by min_confidence
        findings = [f for f in all_findings if f.get("confidence", 100) >= min_confidence]
        ...
        details = {
            ...
            "headers_summary": {
                ...
                "issues": len([f for f in raw_findings if f["severity"] not in ("info",)]),
                "passed": len([f for f in raw_findings if f["severity"] == "info"]),
            },
        }
