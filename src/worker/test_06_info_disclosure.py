...
- Enhanced severity matrix based on actual risk (not just path names)
- Added: Kubernetes, Docker, Terraform, IDE config exposure detection
- Added: JS route extraction (fetch/axios/XHR patterns)
- Added: Cloud storage detection (S3, GCS, Azure Blob)
- Added: Strong evidence requirements (URL, status, headers, size, content evidence)
- robots.txt/sitemap.xml downgraded to LOW unless sensitive info exposed
- Every finding includes: exact URL, status, headers, size, evidence, CWE, OWASP, PoC
- Aggressive false positive reduction with content verification
- Preserved async model, API compatibility, JSON output, budget/timeout logic
"""
import asyncio
import difflib
import json
...
CWE_MAP = {
    ...
    "backup": "CWE-530",
    "user_enum": "CWE-200",
    "k8s_exposure": "CWE-200",
    "docker_exposure": "CWE-200",
    "terraform_exposure": "CWE-538",
    "ide_config": "CWE-200",
    "js_route": "CWE-200",
    "cloud_storage": "CWE-200",
}
...
OWASP_MAP = {
    ...
    "backup": "A05:2021",
    "user_enum": "A01:2021",
    "k8s_exposure": "A05:2021",
    "docker_exposure": "A05:2021",
    "terraform_exposure": "A01:2021",
    "ide_config": "A01:2021",
    "js_route": "A01:2021",
    "cloud_storage": "A06:2021",
}
...
# High:     Source code / config / API schema exposure
# Medium:   Information that aids attack planning
# Low:      Minor info disclosure, non-sensitive files
# Info:     Technology identification only

SEVERITY_MATRIX = {
    # Critical - direct credential exposure
    ".env": ("critical", 95),
    ".env.production": ("critical", 95),
    ".env.local": ("critical", 95),
    ".env.staging": ("critical", 95),
    ".git/HEAD": ("critical", 95),
    ".git/config": ("critical", 90),
    "wp-config.php": ("critical", 95),
    "config.php": ("critical", 90),
    "credentials.json": ("critical", 95),
    "secrets.json": ("critical", 95),
    "id_rsa": ("critical", 95),
    "id_rsa.pub": ("high", 85),
    "private.key": ("critical", 95),
    ".htpasswd": ("critical", 95),
    "database.sql": ("critical", 95),
    "dump.sql": ("critical", 95),
    "db.sql": ("critical", 95),
    "aws-exports.js": ("critical", 90),
    "firebase.json": ("high", 85),
    "supabase.json": ("high", 85),
    ".npmrc": ("critical", 90),
    ".pypirc": ("critical", 90),
    "config/secrets.yml": ("critical", 95),
    "config/database.yml": ("high", 85),
    "parameters.yml": ("high", 85),
    "web.config": ("high", 85),
    "appsettings.json": ("high", 85),
    # High - source code / config / API schema
    "source_map": ("high", 90),
    "graphql_introspection": ("high", 95),
    "swagger_openapi": ("high", 90),
    ".vscode/settings.json": ("high", 85),
    ".vscode/launch.json": ("high", 85),
    ".idea/workspace.xml": ("high", 85),
    "terraform.tfstate": ("critical", 95),
    "terraform.tfvars": ("critical", 95),
    ".terraform/": ("high", 85),
    "docker-compose.yml": ("high", 85),
    "Dockerfile": ("medium", 75),
    ".docker/config.json": ("critical", 95),
    "k8s_api": ("critical", 95),
    "k8s_version": ("medium", 70),
    # Medium - info that aids attack planning
    "phpinfo": ("high", 90),
    "error.log": ("medium", 75),
    "debug.log": ("medium", 75),
    "dir_listing": ("medium", 80),
    "api_doc": ("medium", 75),
    "robots_sensitive": ("medium", 75),
    "backup_file": ("high", 85),
    "composer.json": ("low", 65),
    "package.json": ("low", 65),
    # Low - minor info
    "accessible_file": ("low", 60),
    "forbidden_path": ("low", 55),
    "robots_txt": ("low", 50),
    "sitemap_xml": ("low", 50),
    "tech_disclosure": ("info", 80),
    "user_enum": ("medium", 85),
}
...
def _make_finding(title, description, severity, confidence, location="", evidence="",
    ...
    if response_headers:
        # Limit headers to avoid huge output
    ...
    if extracted_info:
        finding["extracted_info"] = extracted_info[:500]  # cap evidence size
    return finding
...
# Organized by actual risk (not just name)
...
HIGH_SENSITIVE_PATHS = {
    ...
    ".env.local": ("critical", 95),
    ".env.staging": ("critical", 95),
    ".env.development": ("high", 85),
    ".env.backup": ("critical", 95),
    "wp-config.php": ("critical", 95),
    "wp-config.php~": ("critical", 95),
    "wp-config.php.save": ("critical", 95),
    "wp-config.php.bak": ("critical", 95),
    "config.php": ("critical", 90),
    "settings.py": ("high", 85),
    "credentials.json": ("critical", 95),
    "secrets.json": ("critical", 95),
    "secret_key.txt": ("critical", 95),
    "id_rsa": ("critical", 95),
    "id_rsa.pub": ("high", 85),
    "authorized_keys": ("critical", 95),
    "private.key": ("critical", 95),
    "cert.pem": ("high", 85),
    ".htpasswd": ("critical", 95),
    "database.sql": ("critical", 95),
    ...
    "firebase.json": ("high", 85),
    "supabase.json": ("high", 85),
    "vercel.json": ("medium", 70),
    "google-services.json": ("high", 85),
    "sftp-config.json": ("critical", 90),
    ".npmrc": ("critical", 90),
    ".yarnrc.yml": ("medium", 65),
    ".pypirc": ("critical", 90),
    "config/database.yml": ("high", 85),
    "config/secrets.yml": ("critical", 95),
    "parameters.yml": ("high", 85),
    "services.yml": ("medium", 70),
    "web.config": ("high", 85),
    "appsettings.json": ("high", 85),
    # Kubernetes / Docker / Terraform
    "terraform.tfstate": ("critical", 95),
    "terraform.tfstate.backup": ("critical", 95),
    "terraform.tfvars": ("critical", 95),
    ".terraform/terraform.tfstate": ("critical", 95),
    "docker-compose.yml": ("high", 85),
    "docker-compose.yaml": ("high", 85),
    ".docker/config.json": ("critical", 95),
    ".docker/.env": ("critical", 95),
    # IDE configs (often contain secrets)
    ".vscode/settings.json": ("high", 85),
    ".vscode/launch.json": ("high", 85),
    ".vscode/tasks.json": ("medium", 70),
    ".idea/workspace.xml": ("high", 85),
    ".idea/dataSources.xml": ("high", 85),
    ".idea/httpRequests/": ("medium", 70),
}
...
MEDIUM_SENSITIVE_PATHS = {
    ...
    "error.log": ("medium", 75),
    "debug.log": ("medium", 75),
    "storage/logs/laravel.log": ("medium", 75),
    "log/production.log": ("medium", 75),
    "wp-content/debug.log": ("medium", 75),
    "composer.json": ("low", 65),
    "composer.lock": ("low", 60),
    "package.json": ("low", 65),
    "yarn.lock": ("low", 60),
    "pnpm-lock.yaml": ("low", 60),
    "Gemfile": ("low", 60),
    "Gemfile.lock": ("low", 60),
    "requirements.txt": ("low", 60),
    "Dockerfile": ("medium", 75),
    ".gitlab-ci.yml": ("medium", 70),
    ".travis.yml": ("medium", 70),
    "Jenkinsfile": ("medium", 70),
    ".github/workflows/": ("medium", 70),
    ".circleci/config.yml": ("medium", 70),
    "swagger-ui.html": ("high", 85),
    "swagger.json": ("high", 85),
    "v2/api-docs": ("high", 85),
    "v3/api-docs": ("high", 85),
    "api-docs": ("high", 85),
    "graphql": ("high", 85),
...
LOW_SENSITIVE_PATHS = {
    "robots.txt": ("low", 50),
    "sitemap.xml": ("low", 50),
    ".well-known/security.txt": ("low", 55),
    ".well-known/openid-configuration": ("medium", 70),
    "crossdomain.xml": ("low", 50),
    "clientaccesspolicy.xml": ("low", 50),
    "server-status": ("medium", 75),
    "server-info": ("medium", 75),
    "status": ("low", 60),
    "healthcheck": ("low", 55),
    ".DS_Store": ("low", 60),
    "Thumbs.db": ("low", 55),
    ".bash_history": ("high", 85),
    "tmp/restart.txt": ("low", 50),
}
...
def _get_path_severity(path):
    """Get severity and confidence for a path based on the matrix."""
    path_lower = path.lower().lstrip('/')
    if path_lower in HIGH_SENSITIVE_PATHS:
        return HIGH_SENSITIVE_PATHS[path_lower]
    if path_lower in MEDIUM_SENSITIVE_PATHS:
        return MEDIUM_SENSITIVE_PATHS[path_lower]
    if path_lower in LOW_SENSITIVE_PATHS:
        return LOW_SENSITIVE_PATHS[path_lower]
    # Pattern matching for variants
    if path_lower.endswith((".bak", ".backup", ".old", ".orig", ".save", "~")):
        return ("high", 80)
    if any(ext in path_lower for ext in BACKUP_EXTS):
        return ("high", 85)
    if ".sql" in path_lower:
        return ("critical", 90)
    if any(x in path_lower for x in [".env", "secret", "credential", "password", "private"]):
        return ("critical", 90)
    if any(x in path_lower for x in [".git", ".svn", ".hg"]):
        return ("critical", 90)
    if any(x in path_lower for x in [".vscode", ".idea"]):
        return ("high", 80)
    if any(x in path_lower for x in ["terraform", "k8s", "kubernetes", "docker"]):
        return ("high", 85)
    return ("low", 60)
    ...
    try:
        ...
        async def check_js_map(js_url):
            ...
            if resp_map and resp_map.status == 200:
                ...
                if body.strip() and ('"sources"' in body or '"mappings"' in body):
                    # Extract source file count as evidence
                    sources_count = body.count('"sources"')
                    file_count = len(re.findall(r'"file":\s*"[^"]+"', body))
                    return _make_finding(
                        ...
                        category="js", detection_method="source_map_probe",
        ...
        for result in results:
            if isinstance(result, Exception):
                _log_error(f"JS source map check failed for one URL: {result}")
...
# -- Enhanced JS Credential Leak Detection ----------------------------------------------
async def _check_js_leaked_credentials(session, base_url, html, rate_limiter, findings, js_cache):
    ...
    try:
        ...
        for result in results:
            if isinstance(result, Exception):
                _log_error(f"JS leaked credential check failed for one URL: {result}")
...
# -- NEW: JS Route Extraction ------------------------------------------------------------
async def _extract_js_routes(session, base_url, html, rate_limiter, findings, js_cache):
    """Extract API routes from JavaScript files (fetch, axios, XHR patterns)."""
    if not html:
        return
    try:
        soup = BeautifulSoup(html, "html.parser")
        ...
        def extract_from_content(content, source_label):
            for pattern in JS_ROUTE_PATTERNS:
                for match in pattern.finditer(content):
                    ...
                    if any(x in route.lower() for x in ['example.com', 'localhost', '${', 'http://', 'https://']):
                        continue
                    if len(route) > 200:
                        continue
        ...
        # Report unique routes (limit to avoid spam)
        routes_by_path = defaultdict(list)
        for route, source in extracted_routes:
            routes_by_path[route].append(source)

        for route, sources in list(routes_by_path.items())[:20]:  # cap at 20 unique routes
            # Verify route exists by probing
            ...
            try:
                ...
                if resp and resp.status in (200, 401, 403, 405):
                    ...
                    findings.append(_make_finding(
                        ...
                        category="api", detection_method="js_route_extraction",
...
# -- Enhanced API Endpoint Enumeration ---------------------------------------------------
async def _enumerate_api_endpoints(session, base_url, rate_limiter, findings):
    try:
            ...
            if resp.status == 200:
                ...
                if endpoint == "/wp-json/wp/v2/users":
                    if '"id"' in body and '"name"' in body and '"slug"' in body:
                        # Extract user names as evidence
                        ...
                        return _make_finding(
                            ...
                            category="api", detection_method="pattern_match",
                            response_status=resp.status,
                            response_headers=dict(resp.headers) if hasattr(resp, 'headers') else None,
                            response_size=len(body),
                ...
                if 'graphql' in endpoint.lower():
                    try:
                        ...
                        _log_error(f"GraphQL check failed for {endpoint}: {e}")
                    return None
                ...
                if any(kw in body.lower() for kw in ['swagger', 'openapi', 'api-docs']):
                    ...
                    if is_openapi:
                        ...
                        return _make_finding(
                            ...
                            category="api", detection_method="openapi_validation",
                            response_status=resp.status,
                            response_headers=dict(resp.headers) if hasattr(resp, 'headers') else None,
                            response_size=len(body),
        ...
        for result in results:
            if isinstance(result, Exception):
                _log_error(f"API endpoint check failed for one endpoint: {result}")
            elif result is not None:
                findings.append(result)
    except Exception as e:
...
async def _check_kubernetes_exposure(session, base_url, rate_limiter, findings):
        ...
        try:
            ...
            if resp and resp.status == 200:
                try:
                    ...
                    return None
                # Validate it's actually Kubernetes
                if any(kw in body.lower() for kw in ['kind', 'apiversion', 'metadata', 'kubernetes']):
                    ...
                    if kind_match:
                        extracted.append(f"Kind: {kind_match.group(1)}")
                    return _make_finding(
                        ...
                        category="infrastructure", detection_method="k8s_api_probe",
                        response_status=resp.status,
                        response_headers=dict(resp.headers) if hasattr(resp, 'headers') else None,
                        response_size=len(body),
    ...
    for result in results:
        if isinstance(result, Exception):
            _log_error(f"K8s check failed: {result}")
        elif result is not None:
            findings.append(result)


# -- NEW: Docker / Terraform Exposure Detection ------------------------------------------
async def _check_docker_terraform(session, base_url, rate_limiter, findings):
    """Detect Docker and Terraform configuration exposure."""
    paths = [
            ...
            if resp and resp.status == 200:
                ...
                if "docker-compose" in path:
                    if "version:" in body or "services:" in body:
                        ...
                        if services:
                            extracted.append(f"Services: {', '.join(services[:5])}")
                    ...
                    try:
                        ...
                        if "auths" in data or "credsStore" in data:
                            is_valid = True
                            if "auths" in data:
                                extracted.append(f"Registries: {', '.join(data['auths'].keys())}")
                    except Exception:
                        pass
                    ...
                    if re.search(r'^FROM\s+', body, re.MULTILINE):
                        ...
                        if from_match:
                            extracted.append(f"Base image: {from_match.group(1)}")
                    ...
                    if any(kw in body for kw in ['"version"', '"serial"', '"lineage"', 'resource "', 'variable "']):
                        ...
                        if resources:
                            extracted.append(f"Resources: {', '.join(set(resources)[:5])}")
                ...
                if is_valid:
                    ...
                    return _make_finding(
                        ...
                        poc=f"curl -s {url} | head -50",
                        category="infrastructure", detection_method="content_validation",
                        response_status=resp.status,
                        response_headers=dict(resp.headers) if hasattr(resp, 'headers') else None,
                        response_size=len(body),
                        extracted_info="; ".join(extracted) if extracted else None,
    ...
    for result in results:
        if isinstance(result, Exception):
            _log_error(f"Docker/Terraform check failed: {result}")
        elif result is not None:
            findings.append(result)


# -- NEW: Cloud Storage Detection --------------------------------------------------------
async def _check_cloud_storage(session, base_url, html, rate_limiter, findings):
    """Detect exposed cloud storage buckets (S3, GCS, Azure Blob)."""
    # Extract bucket URLs from HTML
    bucket_urls = set()
    if html:
    ...
    # Also check for common bucket names based on domain
        ...
        try:
            ...
            if resp and resp.status == 200:
                ...
                if 'application/xml' in resp.headers.get('Content-Type', ''):
                    ...
                    if 'Index of' in body or '<table' in body:
                        is_listing = True
                ...
                if is_listing:
                    ...
                    return _make_finding(
                        ...
                        category="infrastructure", detection_method="cloud_storage_probe",
                        response_status=resp.status,
                        response_headers=dict(resp.headers) if hasattr(resp, 'headers') else None,
                        response_size=len(body),
    ...
    for result in results:
        if isinstance(result, Exception):
            _log_error(f"Cloud storage check failed: {result}")
        elif result is not None:
            findings.append(result)
...
async def _check_interesting_files_from_headers(session, base_url, main_headers, rate_limiter, findings):
    ...
    async def check_file(path):
        ...
        if resp and resp.status == 200:
            return _make_finding(
                ...
                poc=f"curl -I {url}",
                category="active", detection_method="header_check",
                response_status=resp.status,
                response_headers=dict(resp.headers) if hasattr(resp, 'headers') else None,
                exact_url=url,
            )
    ...
    for result in results:
        if isinstance(result, Exception):
            _log_error(f"Interesting file check failed for {path}: {result}")
        elif result is not None:
            findings.append(result)


# -- Helpers for path probing ------------------------------------------------------------
def _is_sensitive_content(path, body):
    ...
    if "terraform" in path.lower():
        return any(kw in body for kw in ['"version"', '"serial"', 'resource "', 'variable "'])
    if "docker" in path.lower():
        return any(kw in body for kw in ['version:', 'services:', 'FROM ', '"auths"'])
    return False
...
async def run(url: str, shared_page: dict = None):
    ...
    async def main_work():
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                await asyncio.wait_for(
                    asyncio.gather(
                        ...
                        _check_js_leaked_credentials(session, base_url, main_body, rate_limiter, findings, js_cache),
                        ...
                        _extract_js_routes(session, base_url, main_body, rate_limiter, findings, js_cache),
                ...
                _log_error("[FIX #2] Pre-phase timeout after 15s; continuing with partial pre-phase results")
            finally:
            ...
            try:
                        ...
                        try:
                            ...
                            if status == 200:
                                ...
                                if _is_sensitive_content(path, body):
                                    # Upgrade severity for actual sensitive content
                                ...
                                if _is_directory_listing(body, main_body):
                                    findings.append(_make_finding(
                                        ...
                                        poc=f"curl -s {url} | head -50",
                                        category="active", detection_method="listing_detection",
                                        response_status=status,
                                        response_headers=resp_headers,
                                        response_size=len(body),
                                        exact_url=url,
                                ...
                                if path == "robots.txt":
                                    ...
                                    if sensitive_paths:
                                        findings.append(_make_finding(
                                            ...
                                            poc=f"curl -s {url}",
                                            category="active", detection_method="robots_analysis",
                                            response_status=status,
                                            response_headers=resp_headers,
                                            response_size=len(body),
                                    ...
                                    # sitemap.xml is low severity unless it reveals sensitive paths
                                    ...
                                    findings.append(_make_finding(
                                        ...
                                        confidence=70 if sensitive else 55,
                                        location=path,
                                        evidence=f"HTTP {status} response, size: {len(body)} bytes",
                                        remediation="Review if exposure is intentional.",
                                ...
                                elif not _is_soft_404(body, main_body) and path not in ["/robots.txt", "/sitemap.xml", "/favicon.ico"]:
                                    findings.append(_make_finding(
                                        title=f"Accessible file: {path}",
                ...
                _log_error(f"[FIX #1/#3] Path probing budget reached: {paths_checked}/{paths_total} paths checked")
            ...
            unique_findings = []
            for f in findings:
                key = (f["title"], f.get("location", ""), f.get("category", ""))
                if key not in seen:
                    seen.add(key)
    ...
    try:
        ...
        _log_error("[FIX #5] Safety net timeout; returning partial results")
