...
async def query_osv(lib_name: str, version: str, ecosystem: str, session: aiohttp.ClientSession) -> List[dict]:
    ...
    async with osv_sem:
        try:
            async with session.post(OSV_API_URL, json=payload, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
...
async def run(
    ...
    cve_csv_url: str = None  # Kept for backwards compatibility, ignored in favor of OSV
) -> Dict[str, Any]:
    target = url.strip()
    ...
    async with aiohttp.ClientSession(connector=connector, headers=headers) as session:
        for lib, entries in lib_groups.items():
            ...
            if not ver_str or _safe_version(ver_str) is None:
                continue
            ...
            # Query OSV
            osv_vulns = await query_osv(pkg_name, ver_str, ecosystem, session)
            ...
            for vuln in osv_vulns:
                ...
                severity = "medium"
                if cvss:
                    # Extract base score from CVSS vector if possible, or use string
