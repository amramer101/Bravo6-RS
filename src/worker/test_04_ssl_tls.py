...
def _make_finding(*args, **kwargs):
    _expected_params = [
        "title", "description", "severity", "confidence", "location",
        "evidence", "remediation", "cwe", "owasp", "poc", "category"
    ]
    _defaults = [
...
async def run(url: str, shared_page: dict = None) -> Dict[str, Any]:
    ...
    if tls_versions.get("TLSv1.3") and not any(tls_versions.get(v) for v in ["TLSv1.2", "TLSv1.1", "TLSv1.0"]):
        findings.append(_make_finding(
            ...
            "info", 100, location=f"TLS Configuration {hostname}:{port}",
        ...
        findings.append(_make_finding(
            ...
            "critical", 100, location=f"TLS Configuration {hostname}:{port}",
    ...
    if ja3s:
        findings.append(_make_finding(
            ...
            "info", 100, location=f"TLS Handshake {hostname}:{port}",
    ...
    if "h3=" in alt_svc or "h3-29=" in alt_svc or "h3-q050=" in alt_svc or "h3-32=" in alt_svc:
        ...
        findings.append(_make_finding(
            ...
            "info", 100, location=f"Alt-Svc Header {hostname}:{port}",
    ...
    if caa_records:
        findings.append(_make_finding(
            ...
            "info", 100, location=f"DNS CAA for {hostname}",
    ...
    if tlsa_records:
        findings.append(_make_finding(
            ...
            "info", 100, location=f"DNS TLSA for {tlsa_name}",
