...
This document is the single, authoritative reference for the Bravo6 platform's cloud architecture on Microsoft Azure. It maps every service, every resource, and every cost driver in one place, and links out to the dedicated deep-dive documents — data flow, security, and cost — for teams that need more detail in a specific area. Anyone joining the project should start here before moving to the specialised documents.
...
6. [Cost Profile Summary](#6-cost-profile-summary)
...
For the exact Terraform resources behind this topology, see the [Terraform Infrastructure README](./infrastructure.md), in particular the Resource Dependency Graph section.
...
## 4. Network Design in Depth

> The step-by-step request lifecycle (sequence diagram, timing, and polling behaviour) lives in the [Data Flow and Sequence Diagram](./data-flow.md) document. The identity, RBAC, and least-privilege model behind this network design lives in the [Security and Identity Model](./security-identity.md) document. This section covers the network layout only.
...
## 5. Master Component Reference Table
...
## 10. Architecture Decision Records — Snapshot
...
## 12. Navigation and Further Reading
