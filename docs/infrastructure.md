...
graph TD
    subgraph PublicZone[Public Zone - Internet Facing]
        User[Browser / User]
        Frontend[Frontend Storage<br>public_network = true<br>Static Website]
        API[API Function<br>public_network = true<br>HTTP Trigger]
        Report[Report Function<br>public_network = true<br>HTTP Trigger]
    end

    subgraph PrivateZone[Private Zone - VNet Isolated]
        Worker[Worker Function<br>public_network = false<br>VNet Outbound Only]
        SB[Service Bus Queue<br>default_action = Deny<br>Premium SKU]
        Cosmos[Cosmos DB<br>default_action = Deny<br>Free Tier]
        BackendStg[Backend Storage<br>default_action = Deny<br>Function Code]
    end

    User -->|1. GET /| Frontend
    Frontend -->|2. Serve App| User

    User -->|3. POST /api/scan| API
    User -->|4. GET /report/status| Report

    Frontend -.->|AJAX Calls| API
    Frontend -.->|Polling| Report

    API -->|5. Publish Message| SB
    Worker -->|6. Consume Message| SB
    Worker -->|7. Write Results| Cosmos
    Report -->|8. Read Results| Cosmos
